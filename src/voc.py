...
voc12urls = {
    ...
    'test_images_2012': 'http://host.robots.ox.ac.uk/pascal/VOC/voc2012/VOCtest-2012.tar',
}
...
def read_object_csv(file, header=True):
    images = []
    num_categories = 0
    print('[dataset] read', file)
    with open(file, 'r') as f:
...
def download_voc2007(root):
    ...
    if not os.path.exists(path_devkit):
        ...
        parts = parse.urlparse(voc07urls['devkit'])
        filename = os.path.basename(parts.path)
        cached_file = os.path.join(tmpdir, filename)

        if not os.path.exists(cached_file):
            print('Downloading: "{}" to {}\n'.format(voc07urls['devkit'], cached_file))
            util.download_url(voc07urls['devkit'], cached_file)
    ...
    if not os.path.exists(path_images):
        ...
        parts = parse.urlparse(voc07urls['trainval_2007'])
        filename = os.path.basename(parts.path)
        cached_file = os.path.join(tmpdir, filename)

        if not os.path.exists(cached_file):
            print('Downloading: "{}" to {}\n'.format(voc07urls['trainval_2007'], cached_file))
            util.download_url(voc07urls['trainval_2007'], cached_file)
    ...
    if not os.path.exists(test_anno):
        ...
        parts = parse.urlparse(voc07urls['test_images_2007'])
        filename = os.path.basename(parts.path)
        cached_file = os.path.join(tmpdir, filename)

        if not os.path.exists(cached_file):
            print('Downloading: "{}" to {}\n'.format(voc07urls['test_images_2007'], cached_file))
            util.download_url(voc07urls['test_images_2007'], cached_file)
    ...
    if not os.path.exists(test_image):
        ...
        parts = parse.urlparse(voc07urls['test_anno_2007'])
        filename = os.path.basename(parts.path)
        cached_file = os.path.join(tmpdir, filename)

        if not os.path.exists(cached_file):
            print('Downloading: "{}" to {}\n'.format(voc07urls['test_anno_2007'], cached_file))
            util.download_url(voc07urls['test_anno_2007'], cached_file)
...
def download_voc2012(root):
    ...
    if not os.path.exists(path_images):
        ...
        # download train/val images/annotations
        parts = parse.urlparse(voc12urls['trainval_2012'])
        filename = os.path.basename(parts.path)
        cached_file = os.path.join(tmpdir, filename)
    ...
    if not os.path.exists(test_anno):
        ...
        if not os.path.exists(cached_file):
            print('Downloading: "{}" to {}\n'.format(voc12urls['test_images_2007'], cached_file))
    ...
    test_image = os.path.join(path_devkit, 'VOC2012/JPEGImages/2008_000001.jpg')
    if not os.path.exists(test_image):

        # download test images
        parts = urlparse(voc12urls['test_anno_2007'])
        filename = os.path.basename(parts.path)
        cached_file = os.path.join(tmpdir, filename)
...
# class Voc2007ClassificationTest(data.Dataset):
#     def __init__(self, root, set, transform=None, target_transform=None):  
#         #, inp_name=None, adj=None):
#         self.root = root
#         self.path_devkit = os.path.join(root, 'VOCdevkit')
#         self.path_images = os.path.join(root, 'VOCdevkit', 'VOC2007', 'JPEGImages')
#         self.set = set
...
#         self.classes = object_categories
#         self.images = read_object_csv(file_csv)
#         '''
#         with open(inp_name, 'rb') as f:
#             self.inp = pickle.load(f)
#         self.inp_name = inp_name
...
class Voc2012ClassificationTest(data.Dataset):
    def __init__(self, root, set, transform=None, target_transform=None):  
        self.root = root
        self.path_devkit = os.path.join(root, 'VOCdevkit')
        self.path_images = os.path.join(root, 'VOCdevkit', 'VOC2012', 'JPEGImages')
