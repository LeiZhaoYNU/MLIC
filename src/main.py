...
def main():
    ...
    # define dataset,修改voc2007 coco2014
    if args.dataset_name == 'voc2007':
       train_dataset = Voc2007Classification(args.data_path, 'trainval',inp_name='data/voc/voc_glove_word2vec.pkl')
       val_dataset = Voc2007Classification(args.data_path, 'test', inp_name='data/voc/voc_glove_word2vec.pkl')
       num_classes = 20
    ...
    if args.dataset_name == 'coco2014':
       train_dataset = COCO2014(args.data_path, phase='train', inp_name = 'data/coco/coco_glove_word2vec.pkl') 
       val_dataset = COCO2014(args.data_path, phase='val', inp_name = 'data/coco/coco_glove_word2vec.pkl') 
       num_classes = 80
       adj_file = 'data/coco/coco_adj.pkl'
 
    # load model，修改resnet101
    if args.bm == 'resnet101':
       # 加入了三个指标
       model = mcar_resnet101(num_classes=num_classes, ps=args.ps, topN=args.topN, threshold=args.threshold, pretrained=True, in_channel=300, t=0.4, adj_file=adj_file)
   ...
   #  criterion = nn.BCELoss()  #sigmoid映射到[0,1]采用BCE Loss
   ## gcn映射后选择该函数
